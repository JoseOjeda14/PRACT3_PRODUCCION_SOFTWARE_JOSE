...
def test_get_by_id_returns_expense():
    repo = InMemoryExpenseRepository()
    expense = create_expense(id=1, title="Updated", amount=20)
    repo.save(expense)

    expense_1 = repo.get_by_id(1)

    assert expense_1 is not None
    assert expense_1.id == 1
    assert expense_1.title == "Updated"
    assert expense_1.amount == 20
...
def test_get_by_id_returns_none_if_not_found():
    ...
    expense = create_expense(id=1, title="Updated", amount=20)
    repo.save(expense)
    expense_prueba = repo.get_by_id(2)
    assert expense_prueba is None
